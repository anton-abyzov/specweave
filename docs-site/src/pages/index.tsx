...
export default function Home() {
  ...
  return <Layout title="Change agents. Keep the thread." description="Open-source work continuity for AI-assisted engineering. Track intent, link specifications, verify progress, and continue across coding agents.">
    <main className={styles.page}>
      ...
      <section className={`${styles.section} ${styles.showcaseSection}`} id="built-with" aria-labelledby="built-with-title">
        <div className={styles.showcaseHeading}><div><span className={styles.eyebrow}>05 / In practice</span><h2 id="built-with-title">Built with SpecWeave.</h2></div><p>Products where we use the workflow to plan changes, track delivery, and carry work between sessions.</p></div>
        <div className={styles.showcaseProjects}>
          <a href="https://easychamp.com"><span className={styles.showcaseCategory}>Sports platforms</span><h3>EasyChamp <span aria-hidden="true">↗</span></h3><p>League and club tools. SpecWeave supports ongoing work on an established platform.</p></a>
          <a href="https://jobweave.ai"><span className={styles.showcaseCategory}>Career tools</span><h3>JobWeave <span aria-hidden="true">↗</span></h3><p>Job search, tailored resumes, recruiter context, and interview preparation.</p></a>
          <a href="https://verified-skill.com"><span className={styles.showcaseCategory}>Developer tools</span><h3>Verified Skills <span aria-hidden="true">↗</span></h3><p>Skill discovery, source inspection, and local evaluations with Skill Studio.</p></a>
        </div>
