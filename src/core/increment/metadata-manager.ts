...
/**
 * Result of validateMetadataSchema: corrected metadata + any warnings emitted.
 */
 ...
 */
const TYPE_ALIAS_MAP: Record<string, IncrementType> = {
...
export class MetadataManager {
   ...
   */
  static suppressTransitionNotices = false;
  ...
  static read(incrementId: string, rootDir?: string): IncrementMetadata {
    ...
    try {
      ...
      const rawMetadata: any = JSON.parse(content);

      // Validate & auto-correct schema issues (T-012/T-013)
      const { metadata, corrected, warnings } = this.validateMetadataSchema(
        rawMetadata,
        incrementId
      );
  ...
  static updateStatus(
    ...
    (metadata as IncrementMetadata & { updated?: string }).updated = metadata.lastActivity;
    ...
    try {
      ...
      throw new MetadataError(
        `Cannot update increment status - spec.md sync failed.\n` +
          ...
          `\n` +
          `To check for desyncs, run: sw:sync-status`,
        incrementId,
  ...
  private static validateMetadataSchema(
    ...
    if (metadata.type) {
      const alias = TYPE_ALIAS_MAP[metadata.type as string];
      if (alias) {
        warnings.push(`Non-standard type '${metadata.type}' mapped to '${alias}'`);
        metadata.type = alias;
        corrected = true;
      } else if (!Object.values(IncrementType).includes(metadata.type as IncrementType)) {
    ...
    if (!hasNewGithubData && hasOldGithubData) {
      const issues: Record<string, { issueNumber: number; issueUrl: string; status: string }> = {};
  ...
  static getByProvider(provider: 'github' | 'jira' | 'ado'): IncrementMetadataV2[] {
    return (this.getAll() as IncrementMetadataV2[]).filter(
      m => m.syncTarget?.provider === provider
    );
  }
  ...
  static getByProfile(profileId: string): IncrementMetadataV2[] {
    return (this.getAll() as IncrementMetadataV2[]).filter(
      m => m.syncTarget?.profileId === profileId
    );
  }
}
