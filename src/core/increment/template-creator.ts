...
export async function createIncrementTemplates(
  ...
  try {
    ...
    // 5. Create rubric placeholder — reports belong under reports/, never the increment root
    const reportsPath = path.join(incrementPath, 'reports');
    fs.mkdirSync(reportsPath, { recursive: true });
    const rubricPath = path.join(reportsPath, 'rubric.md');
    ...
    fs.writeFileSync(rubricPath, rubricPlaceholder);
    createdFiles.push('reports/rubric.md');
