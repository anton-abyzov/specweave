...
export function migrateLegacyStatus(raw: Record<string, unknown>): LegacyStatusMigration {
  ...
  const metadata: Record<string, unknown> = { ...raw, status: target };
  if (!mapped) {
    return {
      metadata,
      changed: true,
      note: `Unrecognized status '${status}' migrated to '${target}' (2.0 statuses: ${Object.values(
        IncrementStatus,
      ).join(', ')})`,
    };
  }
  const mappedStatus: IncrementStatus = mapped;
 ...
 */
export function computeTransitionPath(
  from: IncrementStatus,
  to: IncrementStatus
): IncrementStatus[] | null {
  if (from === to) return [];

  const queue: Array<{ status: IncrementStatus; path: IncrementStatus[] }> = [
    { status: from, path: [] }
  ];
  const visited = new Set<IncrementStatus>([from]);
