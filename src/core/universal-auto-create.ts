...
export interface ProviderCreateResult {
  created: Array<{ usId: string; ref: string; url: string }>;
...
function emptyResult(): ProviderCreateResult {
 ...
 */
export function parseUserStories(content: string): UserStoryInfo[] {
...
async function createJiraUserStories(
  incrementId: string,
  featureId: string,
  userStories: UserStoryInfo[],
  existingLinks: Record<string, any>,
  config: UniversalAutoCreateConfig,
...
async function createAdoUserStories(
  incrementId: string,
  featureId: string,
  userStories: UserStoryInfo[],
  existingLinks: Record<string, any>,
  ...
  const { AdoClient } = await import(
    '../integrations/ado/ado-client.js'
  );
  const { getAdoPat } = await import(
    '../integrations/ado/ado-pat-provider.js'
  );
...
async function updateMetadata(
  metadataPath: string,
  results: UniversalAutoCreateResult,
): Promise<void> {
 ...
 */
export async function createExternalIssuesForIncrement(
  incrementId: string,
  specPath: string,
  metadataPath: string,
  config: UniversalAutoCreateConfig,
): Promise<UniversalAutoCreateResult> {
  const result: UniversalAutoCreateResult = {};
  ...
  let existingJiraLinks: Record<string, any> = {};
  let existingAdoLinks: Record<string, any> = {};
  ...
  if (config.sync?.jira?.enabled) {
    try {
      result.jira = await createJiraUserStories(
        incrementId, featureId, userStories, existingJiraLinks, config,
      );
    } catch (err) {
  ...
  if (config.sync?.ado?.enabled) {
    try {
      result.ado = await createAdoUserStories(
        incrementId, featureId, userStories, existingAdoLinks, config,
      );
    } catch (err) {
      const providerResult = emptyResult();
      providerResult.errors.push({
