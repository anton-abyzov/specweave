/**
 * Autopilot Logger
 * Structured logging for autopilot sessions
 */

import * as fs from 'fs';
import * as path from 'path';
import { AutopilotLogEntry, SessionSummary, IncrementCompletionReport } from './types.js';
...
export class AutopilotLogger {
  ...
  private entries: AutopilotLogEntry[] = [];
  ...
  log(
    ...
    baseLogger[level](`[Autopilot] ${event}: ${JSON.stringify(details)}`);
  }
   ...
   */
  private appendToFile(entry: AutopilotLogEntry): void {
    try {
      const line = JSON.stringify(entry) + '\n';
      fs.appendFileSync(this.logFile, line, 'utf-8');
    } catch (error) {
      baseLogger.error(`Failed to append to autopilot log: ${error}`);
    }
  }
  ...
  loadFromFile(): boolean {
    try {
      ...
      baseLogger.error(`Failed to load autopilot log: ${error}`);
      return false;
    }
  }
  ...
  writeSummaryMarkdown(summary: SessionSummary): string {
    ...
    const content = `# Autopilot Session Summary
...
*Generated by SpecWeave Autopilot*
`;
