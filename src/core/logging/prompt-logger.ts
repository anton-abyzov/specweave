 ...
 * - **SECURITY**: Automatically masks credentials before saving
...
`;
   ...
   *
   * SECURITY: Masks all credentials before writing to log file
   */
  private buildPromptEntry(
    ...
    // Mask credentials in prompt text before logging
    const sanitizedPrompt = maskCredentials(promptText);

    let entry = `## ${time} - User Prompt\n\n`;
    entry += `${sanitizedPrompt}\n\n`;
...
These logs contain YOUR prompts with automatic credential masking applied.

**Security Features**:
- Credentials (tokens, passwords, API keys) are automatically masked
- Format: Shows first 4 and last 4 characters (e.g., ghp_****def)
- Protects: GitHub tokens, JIRA credentials, database URLs, etc.

**What's still logged**:
- Code snippets (non-sensitive)
- Personal information (review before sharing)
- Project-specific details

**Recommendation**: Review logs before committing to public repositories.
