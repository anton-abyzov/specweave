...
export class ProjectDetector {
  ...
  detectProject(incrementId: string, spec: ParsedSpec): ProjectContext {
    ...
    // 2. Check per-US **Project**: fields (v0.34.1+ - replaces frontmatter.project)
    // Priority: Per-US fields > frontmatter.project (deprecated)
    const perUSProjects = this.extractPerUSProjects(spec);
    if (perUSProjects.length > 0) {
      // Use the first per-US project as primary
      const primaryProject = perUSProjects[0];
      if (this.projects.has(primaryProject)) {
        const scoreData = scores.get(primaryProject)!;
        scoreData.score += 25; // Highest confidence from per-US field
        scoreData.reasoning.push(`Per-US **Project**: field: "${primaryProject}"`);
      }
    } else if (spec.frontmatter.project) {
      ...
      if (this.projects.has(projectId)) {
        ...
        scoreData.reasoning.push(`Explicit project metadata: "${projectId}" (deprecated - use per-US fields)`);
      }
    }
...
`;
  ...
  /**
   * Extract project IDs from per-US **Project**: fields in spec content
   *
   * v0.34.1: New method to support per-US project targeting (ADR-0140)
   *
   * @param spec - Parsed spec with user stories
   * @returns Array of unique project IDs found in user stories
   */
  private extractPerUSProjects(spec: ParsedSpec): string[] {
    const projects = new Set<string>();

    // Check if spec has content property (raw markdown)
    const content = (spec as any).content || (spec as any).bodyContent || '';
    if (typeof content !== 'string') {
      return [];
    }

    // Extract **Project**: fields from markdown content
