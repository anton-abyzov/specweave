...
🤖 Auto-closed by SpecWeave`;
  ...
  static async closeCompletedIncrementIssues(
    ...
    try {
      ...
      const repoIssues = new Map<string, Set<number>>();
      const issueRepoMap = new Map<number, string>(); // issueNum → "owner/repo"
      ...
      if (extIssues && typeof extIssues === 'object') {
        for (const usId of Object.keys(extIssues)) {
          ...
          if (!repoIssues.has(repoSlug)) repoIssues.set(repoSlug, new Set());
      ...
      if (totalIssues === 0) return result;
