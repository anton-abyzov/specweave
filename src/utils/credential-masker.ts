 ...
 */
export function maskCredentials(text: string, options: MaskOptions = {}): string {
  if (!text) return text;
 ...
 */
export function maskCredentialsInData(data: any, options: MaskOptions = {}): any {
 ...
 * // 'GITHUB_TOKEN=ghp_****\nAPI_KEY=sk_t****123'
 */
export function sanitizeBashOutput(output: string, options: MaskOptions = {}): string {
 ...
 */
export function createSecureLogger(logger: any, options: MaskOptions = {}) {
  return {
    log: (message: string, ...args: any[]) => {
      const maskedMsg = maskCredentials(message, options);
      const maskedArgs = args.map(arg =>
        typeof arg === 'string' ? maskCredentials(arg, options) : arg
      );
 ...
 */
export function maskEnvironment(env: NodeJS.ProcessEnv = process.env, options: MaskOptions = {}): Record<string, string> {
