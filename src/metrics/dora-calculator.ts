...
async function main() {
  try {
    ...
    // Write JSON file
    const jsonOutputPath = path.join(resolveEffectiveRoot(), 'metrics', 'dora-latest.json');
    ...
    const reportOutputPath = path.join(resolveEffectiveRoot(), 'metrics', 'dora-report.md');
