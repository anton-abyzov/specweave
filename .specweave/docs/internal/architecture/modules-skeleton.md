# Module Structure

Total modules: 20

## Module Overview

| Module | Path | Files | LOC | Tests | Docs |
|--------|------|-------|-----|-------|------|
| core | `src/core` | 232 | 53,310 | ❌ | ❌ |
| cli | `src/cli` | 95 | 25,504 | ❌ | ❌ |
| utils | `src/utils` | 46 | 9,598 | ❌ | ❌ |
| templates | `src/templates` | 32 | 0 | ❌ | ✅ |
| init | `src/init` | 28 | 4,581 | ❌ | ❌ |
| adapters | `src/adapters` | 22 | 2,747 | ❌ | ✅ |
| locales | `src/locales` | 19 | 0 | ❌ | ❌ |
| sync | `src/sync` | 12 | 3,407 | ❌ | ❌ |
| integrations | `src/integrations` | 11 | 3,797 | ❌ | ❌ |
| metrics | `src/metrics` | 10 | 1,033 | ❌ | ❌ |
| importers | `src/importers` | 9 | 3,638 | ❌ | ❌ |
| hooks | `src/hooks` | 5 | 668 | ❌ | ❌ |
| living-docs | `src/living-docs` | 4 | 2,213 | ❌ | ❌ |
| config | `src/config` | 3 | 300 | ❌ | ❌ |
| types | `src/types` | 3 | 128 | ❌ | ❌ |
| generators | `src/generators` | 2 | 524 | ❌ | ❌ |
| id-generators | `src/id-generators` | 2 | 220 | ❌ | ❌ |
| validators | `src/validators` | 2 | 471 | ❌ | ❌ |
| progress | `src/progress` | 1 | 125 | ❌ | ❌ |
| testing | `src/testing` | 1 | 424 | ❌ | ❌ |

## Module Details

### core

**Path**: `src/core`

- Files: 232
- Estimated LOC: 53,310
- Has Tests: No
- Has README: No
- Entry Points: `background/index.ts`, `cicd/index.ts`, `config/index.ts`, `dashboard/index.ts`, `discrepancy/analyzers/index.ts`, `discrepancy/index.ts`, `errors/index.ts`, `feature-deleter/index.ts`, `iac/index.ts`, `living-docs/index.ts`, `living-docs/sync-helpers/index.ts`, `llm/index.ts`, `logs/index.ts`, `notifications/index.ts`, `repo-structure/providers/index.ts`, `scheduler/index.ts`, `serverless/index.ts`, `workflow/index.ts`
- File Types: .ts (230), .disabled (1), .json (1)

**Description**: *To be documented*

---

### cli

**Path**: `src/cli`

- Files: 95
- Estimated LOC: 25,504
- Has Tests: No
- Has README: No
- Entry Points: `helpers/init/index.ts`, `helpers/issue-tracker/index.ts`
- File Types: .ts (95)

**Description**: *To be documented*

---

### utils

**Path**: `src/utils`

- Files: 46
- Estimated LOC: 9,598
- Has Tests: No
- Has README: No
- Entry Points: `docs-preview/index.ts`, `validators/index.ts`
- File Types: .ts (46)

**Description**: *To be documented*

---

### templates

**Path**: `src/templates`

- Files: 32
- Estimated LOC: 0
- Has Tests: No
- Has README: Yes
- File Types: .md (13), .template (7), .yml (7), .example (1), .qa (1)

**Description**: *To be documented*

---

### init

**Path**: `src/init`

- Files: 28
- Estimated LOC: 4,581
- Has Tests: No
- Has README: No
- File Types: .ts (27), .stub (1)

**Description**: *To be documented*

---

### adapters

**Path**: `src/adapters`

- Files: 22
- Estimated LOC: 2,747
- Has Tests: No
- Has README: Yes
- File Types: .ts (11), .md (10), .yaml (1)

**Description**: *To be documented*

---

### locales

**Path**: `src/locales`

- Files: 19
- Estimated LOC: 0
- Has Tests: No
- Has README: No
- File Types: .json (11),  (8)

**Description**: *To be documented*

---

### sync

**Path**: `src/sync`

- Files: 12
- Estimated LOC: 3,407
- Has Tests: No
- Has README: No
- Entry Points: `index.ts`
- File Types: .ts (12)

**Description**: *To be documented*

---

### integrations

**Path**: `src/integrations`

- Files: 11
- Estimated LOC: 3,797
- Has Tests: No
- Has README: No
- File Types: .ts (11)

**Description**: *To be documented*

---

### metrics

**Path**: `src/metrics`

- Files: 10
- Estimated LOC: 1,033
- Has Tests: No
- Has README: No
- File Types: .ts (10)

**Description**: *To be documented*

---

### importers

**Path**: `src/importers`

- Files: 9
- Estimated LOC: 3,638
- Has Tests: No
- Has README: No
- File Types: .ts (9)

**Description**: *To be documented*

---

### hooks

**Path**: `src/hooks`

- Files: 5
- Estimated LOC: 668
- Has Tests: No
- Has README: No
- Entry Points: `index.ts`
- File Types: .ts (5)

**Description**: *To be documented*

---

### living-docs

**Path**: `src/living-docs`

- Files: 4
- Estimated LOC: 2,213
- Has Tests: No
- Has README: No
- File Types: .ts (4)

**Description**: *To be documented*

---

### config

**Path**: `src/config`

- Files: 3
- Estimated LOC: 300
- Has Tests: No
- Has README: No
- File Types: .ts (3)

**Description**: *To be documented*

---

### types

**Path**: `src/types`

- Files: 3
- Estimated LOC: 128
- Has Tests: No
- Has README: No
- File Types: .ts (3)

**Description**: *To be documented*

---

### generators

**Path**: `src/generators`

- Files: 2
- Estimated LOC: 524
- Has Tests: No
- Has README: No
- File Types: .ts (2)

**Description**: *To be documented*

---

### id-generators

**Path**: `src/id-generators`

- Files: 2
- Estimated LOC: 220
- Has Tests: No
- Has README: No
- File Types: .ts (2)

**Description**: *To be documented*

---

### validators

**Path**: `src/validators`

- Files: 2
- Estimated LOC: 471
- Has Tests: No
- Has README: No
- File Types: .ts (2)

**Description**: *To be documented*

---

### progress

**Path**: `src/progress`

- Files: 1
- Estimated LOC: 125
- Has Tests: No
- Has README: No
- File Types: .ts (1)

**Description**: *To be documented*

---

### testing

**Path**: `src/testing`

- Files: 1
- Estimated LOC: 424
- Has Tests: No
- Has README: No
- File Types: .ts (1)

**Description**: *To be documented*

---

*Generated by Living Docs Builder on 2025-12-04*