...
```
1. Developer completes task → TodoWrite fires
2. Post-task-completion hook triggers
3. Hook calls sync-spec-commits CLI
4. CLI:
   a. Parses spec.md → Extracts user stories
   b. Parses tasks.md → Extracts tasks with AC-IDs
   c. Maps tasks → user stories (via AC-IDs)
...
Result:
US-001 completed → Post comment with T-001 and abc123
```
...
- **Spec → Code**: Click commit link in external tool → see exact code changes
- **Code → Spec**: Git commit message references spec user story
...
**Test Scenarios**:
1. Complete user story → Verify comment posted
2. WIP commits → Verify short update posted
3. Multiple repos → Verify multiple comments
4. No external tool link → Verify graceful skip
5. Dry run → Verify no actual posting
...
- ✅ 3 provider implementations (GitHub, JIRA, ADO)
