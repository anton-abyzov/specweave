...
async function generateEnvFile(config: EnvConfig): Promise<void> {
  ...
  console.log('\n⚠️  SECURITY WARNING:');
  console.log('   • .env contains your GitHub token (keep it secret!)');
  console.log('   • NEVER commit .env to git');
  console.log('   • Share .env.example with your team instead');
  console.log('   • Regenerate token if accidentally committed\n');
}
```
...
async function detectCommittedEnv(): Promise<void> {
  try {
    ...
    if (result.trim().length > 0) {
      console.error('🚨 CRITICAL: .env file is tracked by git!');
...
**Cons**:
- ❌ Poor UX (manual busywork)
- ❌ Users forget required variables
- ❌ No validation
...
**Cons**:
- ❌ Not standard (most tools use .env)
- ❌ Harder to integrate with other tools
- ❌ JSON doesn't support comments (less user-friendly)
- ❌ Mixing code config with secrets (poor practice)
...
- ❌ Hard to debug
- ❌ Not portable (can't copy to other machines easily)
...
**Cons**:
- ❌ Terrible UX (type token every time)
- ❌ Can't automate (no hooks)
- ❌ Can't run in CI/CD
- ❌ Users will script workarounds (defeating the purpose)
...
**User Experience**:
- ✅ Zero manual setup (auto-generated)
- ✅ Sync works immediately after init
- ✅ Clear documentation (comments in .env)
- ✅ Team collaboration easy (.env.example)

**Security**:
- ✅ Automatic .gitignore updates
- ✅ Restrictive permissions (0600)
- ✅ Clear warnings about sensitive data
- ✅ Detection of accidental commits

**Standards Compliance**:
- ✅ Follows .env conventions (industry standard)
- ✅ Compatible with dotenv libraries
- ✅ Works with CI/CD (.env.example as template)

### Negative

**Security Risks**:
- ❌ File-based storage (less secure than OS keychain)
- ❌ Plaintext tokens (readable if permissions wrong)
- ❌ Risk of accidental commit (mitigated by .gitignore)

**Complexity**:
- ❌ More code to maintain (~150 lines)
- ❌ Multi-provider support complexity
- ❌ File permission handling (OS-specific)

**Edge Cases**:
- ❌ Overwrite confirmation needed if .env exists
- ❌ Multiple .env files (.env.local, .env.production)
- ❌ Path issues (non-standard project layouts)
...
function checkTokenAge(token: string): void {
  ...
  if (tokenAge > 90 * 24 * 60 * 60 * 1000) { // 90 days
    console.warn('⚠️  GitHub token is >90 days old. Consider rotating it.');
