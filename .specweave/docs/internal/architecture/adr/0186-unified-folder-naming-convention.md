...
1. `ado-project-detector.ts:createProjectFolders()` used **raw project names** directly:
   ```typescript
   const projectPath = path.join(specsPath, project);  // "Nova X Sandbox" with spaces!
   ```

2. `external-import.ts:buildAdoConfigFromProjects()` used `normalizeToProjectId()`:
   ```typescript
   const projectFolder = normalizeToProjectId(proj.name);  // "nova-x-sandbox"
   ```

3. `ado.ts:createSingleProjectFolders()` used **raw project names** directly:
   ```typescript
   const projectDir = path.join(specsDir, projectName);  // "Nova X Sandbox" with spaces!
   ```

**Result**: Duplicate folders for the same ADO project:
- `.specweave/docs/internal/specs/Nova X Sandbox/` (from folder creation)
- `.specweave/docs/internal/specs/nova-x-sandbox/` (from import)
...
export function normalizeToProjectId(name: string): string {
  return name
    .toLowerCase()                    // "Nova X Sandbox" → "nova x sandbox"
    .replace(/[^a-z0-9]+/g, '-')      // "nova x sandbox" → "nova-x-sandbox"
...
If you have duplicate folders like "Nova X Sandbox" and "nova-x-sandbox":
...
mv ".specweave/docs/internal/specs/Nova X Sandbox/"* \
   ".specweave/docs/internal/specs/nova-x-sandbox/"

# 3. Remove the space-containing folder
rm -rf ".specweave/docs/internal/specs/Nova X Sandbox"
```
