...
- [ ] **AC-US1-01**: Create `plugins/specweave/hooks/stop-auto.sh` that implements the Stop Hook logic
- [ ] **AC-US1-02**: Stop hook checks `.specweave/state/auto-session.json` for active session state
- [ ] **AC-US1-03**: When auto active, hook blocks exit and re-feeds original prompt with iteration context
...
- [ ] **AC-US1-06**: Completion promise detection: when output contains `<auto-complete>DONE</auto-complete>`, allow exit
