...
**Status**: [ ] pending
**Test**: Given /sw:done command → When increment complete → Then runs npm test and blocks if failing

### T-013: Add Build Validation to /sw:done
**User Story**: US-004 | **Satisfies ACs**: AC-US4-02
**Status**: [ ] pending
**Test**: Given /sw:done command → When increment complete → Then runs npm run build and blocks if failing

### T-014: Add E2E Validation to /sw:done
**User Story**: US-004 | **Satisfies ACs**: AC-US4-03
**Status**: [ ] pending
**Test**: Given /sw:done command → When E2E tests exist → Then runs playwright tests and blocks if failing

### T-015: Add Coverage Validation to /sw:done
**User Story**: US-004 | **Satisfies ACs**: AC-US4-04
**Status**: [ ] pending
**Test**: Given /sw:done command → When coverage config exists → Then validates thresholds

### T-016: Update Auto Mode to Call /sw:done
**User Story**: US-004 | **Satisfies ACs**: AC-US4-05, AC-US4-06
**Status**: [ ] pending
**Test**: Given auto mode → When increment tasks complete → Then calls /sw:done for validation
...
**Status**: [ ] pending
**Test**: Given auto.ts → When removing github sync calls → Then relies on hooks only

### T-018: Remove Manual JIRA Sync Calls
**User Story**: US-005 | **Satisfies ACs**: AC-US5-02
**Status**: [ ] pending
**Test**: Given auto.ts → When removing jira sync calls → Then relies on hooks only

### T-019: Remove Manual Living Docs Sync
**User Story**: US-005 | **Satisfies ACs**: AC-US5-04
**Status**: [ ] pending
**Test**: Given auto.ts → When removing living docs sync → Then relies on hooks only

### T-020: Remove Manual AC Updates
**User Story**: US-005 | **Satisfies ACs**: AC-US5-05
**Status**: [ ] pending
**Test**: Given auto.ts → When removing AC update logic → Then relies on post-task-completion hook
...
**Status**: [ ] pending
**Test**: Given simplified architecture → When updating docs → Then explains framework handles sync

### T-024: Document Architecture Changes
**User Story**: US-001 | **Satisfies ACs**: AC-US1-02
**Status**: [ ] pending
**Test**: Given refactoring → When writing ADR → Then explains why simplification was needed
...
**Status**: [ ] pending
**Test**: Given 3 active increments → When running auto mode → Then completes all and exits gracefully
