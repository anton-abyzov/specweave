#!/usr/bin/env node
/**
 * Sync All Epic Specs to GitHub (WITH PROPER CONTENT!)
 *
 * This script uses EpicContentBuilder to generate proper hierarchical content:
...
async function syncEpic(epicId: string, specsDir: string, projectRoot: string): Promise<SyncResult> {
  ...
  // Build hierarchical GitHub issue body using EpicContentBuilder
  console.log(`   🏗️  Building hierarchical content (User Stories + Tasks)...`);
  const contentBuilder = new EpicContentBuilder(epicFolder, projectRoot);
  const issueBody = await contentBuilder.buildIssueBody();

  // Check if Epic already has GitHub issue
  if (epic.githubIssue) {
    console.log(`   ♻️  Updating existing Issue #${epic.githubIssue}...`);
    await updateGitHubIssue(epic.githubIssue, issueBody);
    return {
      epicId,
  ...
  if (existingIssue) {
    ...
    await updateGitHubIssue(existingIssue, issueBody);
    return {
      epicId,
      epicTitle: epic.title,
      success: true,
  ...
  const { number, url } = await createGitHubIssue(epic.id, epic.title, issueBody);
...
async function syncAllEpics(): Promise<void> {
  console.log('🚀 Syncing All Epic Specs to GitHub (WITH PROPER CONTENT!)\n');
  ...
  for (let i = 0; i < epicFolders.length; i++) {
    ...
    try {
      const result = await syncEpic(epicId, specsDir, projectRoot);
      results.push(result);
      successCount++;
