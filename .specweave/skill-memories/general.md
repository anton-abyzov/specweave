...
- **2026-02-11**: Agent Teams uses --teammate-mode flag (not --dangerously-escape-agent-box) with values: in-process, tmux, or auto. When spawning teammates with team.ts, must pass detected --teammate-mode to claude command arguments, not just log it.
