...
async function syncSpecCommitsToAdo(client, options) {
...
async function postCommitBatchUpdate(client, incrementPath, commits, workItemId, repo, dryRun = false) {
  try {
    for (const commit of commits) {
      commit.url = getCommitUrl(commit, repo);
    }
...
async function getAdoWorkItemForSpec(specPath) {
  try {
