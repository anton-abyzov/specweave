 ...
 */
export async function buildHierarchicalWIQL(organization, pat, containers) {
    ...
    for (const container of containers) {
        ...
        if (container.subOrganizations && container.subOrganizations.length > 0) {
            try {
                const areaPaths = await getAreaPaths(organization, container.id, pat, container.subOrganizations);
                if (areaPaths.length > 0) {
                    ...
                    console.warn(`⚠️  No valid area paths found for project ${container.id}, syncing all areas`);
                }
            }
            catch (error) {
                console.warn(`⚠️  Failed to resolve area paths for ${container.id}, syncing all areas:`, error.message);
            }
        }
...
function buildFilterClauses(filters) {
    ...
    if (filters.iterationPaths && filters.iterationPaths.length > 0) {
        const iterationClauses = filters.iterationPaths
            .map((path) => `[System.IterationPath] UNDER '${path}'`)
            .join(' OR ');
    ...
    if (filters.includeLabels && filters.includeLabels.length > 0) {
        const tagClauses = filters.includeLabels
            .map((tag) => `[System.Tags] CONTAINS '${tag}'`)
            .join(' OR ');
    ...
    if (filters.assignees && filters.assignees.length > 0) {
        const assigneeClauses = filters.assignees
            .map((assignee) => `[System.AssignedTo] = '${assignee}'`)
            .join(' OR ');
 ...
 */
export async function fetchWorkItemsHierarchical(profile, pat, timeRange = '1M') {
...
//# sourceMappingURL=ado-hierarchical-sync.js.map