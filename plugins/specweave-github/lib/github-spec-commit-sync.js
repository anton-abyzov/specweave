...
import { buildUserStoryComment, buildCommitBatchComment, buildShortComment, generateSmartSummary, } from '../../../src/core/comment-builder.js';
import path from 'path';
import fs from 'fs/promises';
/**
 * Sync commits for completed user stories to GitHub issue
 */
export async function syncSpecCommitsToGitHub(options) {
 ...
 */
export async function postCommitBatchUpdate(incrementPath, commits, issueNumber, repo, dryRun = false) {
    try {
        const client = GitHubClientV2.fromRepo(repo.owner, repo.repo);
        // Add commit URLs
        for (const commit of commits) {
 ...
 */
export async function getGitHubIssueForSpec(specPath) {
    try {
...
//# sourceMappingURL=github-spec-commit-sync.js.map