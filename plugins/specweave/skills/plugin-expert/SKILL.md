---
name: plugin-expert
description: Complete authority on Claude Code plugins, marketplaces, skills, agents, hooks, and MCP servers. Activates for plugin development, marketplace creation, skill writing, agent setup, hook configuration, plugin installation, plugin troubleshooting, marketplace management, SKILL.md format, YAML frontmatter, allowed-tools, plugin.json, marketplace.json, extraKnownMarketplaces, .claude-plugin directory, plugin commands, plugin structure. Keywords plugin install, plugin marketplace, claude code plugins, plugin development, skill creation, agent invocation, hook events, MCP integration, marketplace.json, plugin.json, SKILL.md, YAML frontmatter, allowed-tools, extraKnownMarketplaces, plugin structure, plugin manifest, skill activation, hook triggers.
---
...
13. [SpecWeave Plugins](#specweave-plugins)
...
**Result**: Claude Code fetches plugins from GitHub on-demand

**For SpecWeave repo** (development):

```bash
# Add local marketplace (CLI only)
/plugin marketplace add ./.claude-plugin
...
**specweave** - Framework essentials (auto-installed)
- **Skills**: 9 (increment-planner, plugin-expert, etc.)
- **Agents**: 22 (PM, Architect, QA Lead, etc.)
- **Commands**: 22 (/specweave:increment, /specweave:do, etc.)
- **Hooks**: 8 (post-task-completion, user-prompt-submit, etc.)

**Installation**: Automatic during `specweave init`
...
/plugin install specweave-github
```
- GitHub sync, PR automation, issue tracking
- Skills: github-sync, github-issue-tracker, github-multi-project
- Agents: github-manager, github-task-splitter, user-story-updater
- Commands: /specweave-github:sync

**specweave-jira** - Jira integration
```bash
/plugin install specweave-jira
```
- Jira sync, epic management, sprint tracking
- Skills: jira-sync, specweave-jira-mapper
- Agents: jira-manager
- Commands: /specweave-jira:sync

**specweave-ado** - Azure DevOps integration
```bash
/plugin install specweave-ado
```
- ADO work items, boards, sprints
- Skills: ado-sync, ado-multi-project, ado-resource-validator
- Agents: ado-manager, ado-multi-project-mapper, ado-sync-judge
- Commands: /specweave-ado:sync
...
**specweave-backend** - Node.js, .NET, Python backends
```bash
...
```bash
/plugin install specweave-mobile
```
...
**specweave-kafka** - Kafka architecture and workflows
```bash
/plugin install specweave-kafka
```

**specweave-release** - Release management and versioning
```bash
...
```bash
# 1. Initialize SpecWeave (auto-installs core + registers marketplace)
specweave init .
