 ...
 */
export function resolveGitHubToken(projectRoot, options = {}) {
...
export function describeGitHubAuth(auth, login) {
    if (!auth.token)
        return 'GitHub token: none (set GITHUB_TOKEN, add it to .env, or run `gh auth login`)';
 ...
 */
export function resolveGitHubLogin(token, exec = runSync) {
    if (!token)
        return null;
    try {
...
function readGhCliToken() {
    ...
    try {
        ...
        if (fs.existsSync(ghConfigPath)) {
            const config = yaml.load(fs.readFileSync(ghConfigPath, 'utf8'), { schema: yaml.JSON_SCHEMA });
 ...
 */
export function getGitHubAuthFromProject(projectRoot, configToken) {
