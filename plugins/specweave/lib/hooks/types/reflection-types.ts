...
export enum ReflectionDepth {
  ...
  DEEP = 'deep'          // <60s, detailed + architectural
}
...
export enum ReflectionModel {
  ...
  OPUS = 'opus'        // Deep analysis (~$0.15/task)
}
...
export enum ReflectionMode {
  ...
  DISABLED = 'disabled' // Reflection disabled
}
...
export interface ReflectionMetrics {
  codeQuality: number;      // 1-10 rating
  security: number;         // 1-10 rating
  testCoverage?: number;    // Percentage or null
...
export interface ReflectionResult {
  ...
  recommendedActions: {
    priority1: string[];  // MUST FIX
    priority2: string[];  // SHOULD FIX
    priority3: string[];  // NICE TO HAVE
  };
