  ...
  if (items && items.length > 0) {
    ...
    for (const item of toShow) {
      ...
      if (status === 'ready_for_review') {
        console.log(`      → specweave complete ${item}`);
      }
    }
    if (items.length > 5) {
...
if (reviewCount > 0) {
  console.log('💡 Next step: specweave complete <id> to close reviewed increment(s)');
} else if (activeCount > 0) {
  console.log('💡 Commands:');
  console.log('   specweave status   Show task progress');
  console.log('   specweave task next  Pick the next task');
} else {
