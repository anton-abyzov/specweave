...
- [ ] **AC-US1-02**: Second criterion
    ...
    it('should handle missing user story file', async () => {
      vi.mocked(execSync).mockReturnValue('- [x] **AC-US1-01**: Test');
    ...
    it('should validate code exists for completed tasks', async () => {
      vi.mocked(execSync).mockReturnValue('- [x] **T-001**: Complete task');
      vi.mocked(fs.readFile).mockResolvedValue(`
---
id: US-001
---
...
- [ ] **AC-US1-03**: Third
  ...
  describe('syncBidirectional', () => {
    it('should prioritize increment as source of truth', async () => {
      vi.mocked(fs.readFile).mockResolvedValue('- [x] **AC-US1-01**: Test');
...
---
      ...
      vi.mocked(fs.readFile).mockResolvedValue(tasksContent);
      vi.mocked(fs.writeFile).mockResolvedValue(undefined);
