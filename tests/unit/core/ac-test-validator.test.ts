...
describe('ACTestValidator', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `ac-validator-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
...
describe('createACTestValidator', () => {
  ...
  beforeEach(async () => {
    // ✅ SAFE: Isolated test directory with unique ID (prevents race conditions)
    testDir = path.join(os.tmpdir(), `ac-validator-factory-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
    await fs.ensureDir(testDir);
  });
