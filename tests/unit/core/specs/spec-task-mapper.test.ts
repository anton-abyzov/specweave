...
// Note on AC-ID format: The parseAcceptanceCriteriaFromContent function uses
// the raw usNumber digits as captured from **US-NNN**. For **US-001** the
// captured number is "001", so ACs must use AC-US001-NN format. For **US-01**
// the captured number is "01", so ACs use AC-US01-NN.
...
// Note on task completion format: The parseTasks completion regex looks for
// \[(x)\]\s+T-NNN\b anywhere in the content, so we use the format:
// [x] T-001   or a checklist line containing [x] T-NNN
...
`;
  ...
  it('parses multiple user stories with different number of ACs', async () => {
    const content = `# Spec

**US-001**: Story with three ACs
- [x] **AC-US001-01**: First (P1)
- [x] **AC-US001-02**: Second (P1)
- [ ] **AC-US001-03**: Third (P2)

**US-002**: Story with one AC
- [x] **AC-US002-01**: Only one (P1)

**US-003**: Story with no ACs
`;
    mockReadFile.mockResolvedValue(content);
    const result = await parseSpec('/fake/spec.md');
    ...
    expect(result[0].acceptanceCriteria).toHaveLength(3);
    expect(result[1].acceptanceCriteria).toHaveLength(1);
    expect(result[2].acceptanceCriteria).toHaveLength(0);
  });

  it('does not assign ACs from one US to another', async () => {
    const content = `# Spec

**US-001**: First story
- [x] **AC-US001-01**: Belongs to US-001 (P1)

**US-002**: Second story
- [ ] **AC-US002-01**: Belongs to US-002 (P1)
`;
    mockReadFile.mockResolvedValue(content);
    const result = await parseSpec('/fake/spec.md');
...
describe('parseTasks', () => {
  ...
  it('derives user stories from AC-IDs', async () => {
    ...
    // deriveUserStoriesFromACIds extracts the number from AC-US(\d+)- pattern
    // For AC-US001-01, the number is "001", producing "US-001"
  ...
  it('deduplicates user story references', async () => {
    ...
    // T-001 references AC-US001-01 and AC-US001-02, both from US-001
    expect(result[0].userStories).toEqual(['US-001']);
  });

  it('parses completed status from [x] T-NNN pattern', async () => {
    mockReadFile.mockResolvedValue(TASKS_CONTENT_BASIC);
    const result = await parseTasks('/fake/tasks.md');
  ...
  it('stops task section at --- separator', async () => {
    ...
    expect(taskIds).toContain('T-003');
  });
...
`;
  ...
  it('marks task as not completed when [x] T-NNN pattern is absent', async () => {
    const content = `# Tasks
...
`;
  ...
  it('extracts AC-IDs matching AC-USNN-NN pattern only', async () => {
    const content = `# Tasks

## T-001: Task with mixed content
**AC**: AC-US001-01, AC-US001-02, not-valid, AC-US002-01
[ ] T-001
`;
    mockReadFile.mockResolvedValue(content);
    const result = await parseTasks('/fake/tasks.md');
...
describe('createTaskToUserStoryMapping', () => {
  ...
  it('preserves order of tasks in mapping output', () => {
    const tasks: Task[] = [
      { id: 'T-003', title: 'Third', acIds: [], userStories: ['US-001'], completed: false },
      { id: 'T-001', title: 'First', acIds: [], userStories: ['US-001'], completed: true },
      { id: 'T-002', title: 'Second', acIds: [], userStories: ['US-002'], completed: false },
    ];

    const result = createTaskToUserStoryMapping(tasks, sampleUserStories);

    expect(result[0].taskId).toBe('T-003');
    expect(result[1].taskId).toBe('T-001');
    expect(result[2].taskId).toBe('T-002');
  });
});
...
describe('getCompletedUserStories', () => {
  ...
  it('requires ALL related tasks complete for a user story', () => {
    const mapping: SpecTaskMapping = {
      specId: 'spec-001',
      specPath: '/fake/spec.md',
      incrementId: '0001',
...
describe('getSpecTaskMapping', () => {
  ...
  it('extracts spec-NNN from SPEC-NNN- pattern in increment spec', async () => {
    const incrementSpec = `# Spec

**Implements**: SPEC-007-james-bond-feature
`;
    mockReadFile
      .mockResolvedValueOnce(incrementSpec)
      .mockResolvedValueOnce(SPEC_CONTENT_EMPTY)
      .mockResolvedValueOnce(TASKS_CONTENT_EMPTY);

    const result = await getSpecTaskMapping(
      '/project/.specweave/increments/0001-bond',
      '/fake/spec-007.md'
    );

    expect(result).not.toBeNull();
    expect(result!.specId).toBe('spec-007');
  });

  it('sets specId to "unknown" when no SPEC reference found but specPath is given', async () => {
    mockReadFile
      .mockResolvedValueOnce(INCREMENT_SPEC_NO_REF)
      .mockResolvedValueOnce(SPEC_CONTENT_EMPTY)
      .mockResolvedValueOnce(TASKS_CONTENT_EMPTY);

    const result = await getSpecTaskMapping(
      '/project/.specweave/increments/0001-test',
      '/fake/spec.md'
    );
...
describe('integration: parseSpec + parseTasks + createTaskToUserStoryMapping', () => {
  it('creates end-to-end mapping from spec and tasks content', async () => {
    ...
    // T-001 maps to US-001 via AC-US001-01, AC-US001-02
    ...
    // T-003 maps to US-002 via AC-US002-01, AC-US002-02
...
`;
  ...
  it('parseTasks: triple-hash heading matches due to non-anchored regex', async () => {
    const content = `# Tasks
# T-001: Single hash (only one # so no match)
### T-002: Triple hash (matches ## within ###)
## Not a task heading
## T-: Missing number
`;
    mockReadFile.mockResolvedValue(content);
  ...
  it('parseSpec handles AC with unclosed paren in metadata', async () => {
    const content = `# Spec
...
`;
    ...
    expect(result[0].acceptanceCriteria).toHaveLength(2);
    expect(result[0].acceptanceCriteria[0].description).toBe('Description');
    expect(result[0].acceptanceCriteria[0].priority).toBeUndefined();
    expect(result[0].acceptanceCriteria[0].testable).toBe(false);
    expect(result[0].acceptanceCriteria[0].completed).toBe(false);
    expect(result[0].acceptanceCriteria[1].description).toBe('Another');
    expect(result[0].acceptanceCriteria[1].completed).toBe(true);
  });
...
`;
  ...
  it('parseSpec with AC having only testable flag (no priority)', async () => {
    const content = `# Spec

**US-001**: Story
- [ ] **AC-US001-01**: Description (testable)
`;
    mockReadFile.mockResolvedValue(content);
    const result = await parseSpec('/fake/spec.md');

    expect(result[0].acceptanceCriteria[0].testable).toBe(true);
    expect(result[0].acceptanceCriteria[0].priority).toBeUndefined();
  });

  it('parseTasks with single task in content', async () => {
    const content = `# Tasks
...
`;
    ...
    expect(result).toHaveLength(1);
    expect(result[0].id).toBe('T-042');
    expect(result[0].title).toBe('The only task');
    expect(result[0].completed).toBe(true);
    expect(result[0].acIds).toEqual(['AC-US001-01']);
  });

  it('getSpecTaskMapping handles errors in parseSpec gracefully', async () => {
    mockReadFile
      .mockResolvedValueOnce(INCREMENT_SPEC_WITH_REF)
  ...
  it('getSpecTaskMapping handles errors in parseTasks gracefully', async () => {
    mockReadFile
      .mockResolvedValueOnce(INCREMENT_SPEC_WITH_REF)
      .mockResolvedValueOnce(SPEC_CONTENT_BASIC)
      .mockRejectedValueOnce(new Error('Permission denied'));
