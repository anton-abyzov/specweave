...
describe('ADO Hierarchy Grouping - Bug Fix Verification', () => {
  ...
  describe('CRITICAL BUG FIX: Items without parents should group by area path, NOT title', () => {
    ...
    it('should group orphan User Stories by area path, not by title', () => {
      ...
      // User Stories from OlySense project with various titles but same area path
      const items: ExternalItem[] = [
        createMockItem({
          id: 'ADO-1001',
          title: '401 error encountered during authentication',
          adoProjectName: 'OlySense',
          adoAreaPath: 'OlySense\\Platform-Engineering',
  ...
  describe('Edge cases', () => {
    ...
    it('should handle items with only project name in area path', () => {
      // Area path is just "OlySense" without sub-path
      const items: ExternalItem[] = [
        createMockItem({
          id: 'ADO-600',
          title: 'Root level task',
          adoProjectName: 'OlySense',
          adoAreaPath: 'OlySense', // Just project name
        }),
      ];
      ...
      expect(groups[0].projectId).toBe('olysense');
    });
    ...
    it('should handle deeply nested area paths', () => {
      ...
      const items: ExternalItem[] = [
        createMockItem({
          ...
          adoProjectName: 'OlySense',
          adoAreaPath: 'OlySense\\Engineering\\Platform\\Core',
        }),
      ];
...
describe('Judge Pattern Validation', () => {
  ...
  it('should detect title-based folder names as errors', () => {
    ...
    const buggyGroups: ContainerGroup[] = [
      {
        containerId: 'OlySense',
        containerType: 'ado',
        projectId: '401-error-encountered-during-authentication', // BUG!
        items: [],
        externalContainer: { type: 'ado-project', containerId: 'OlySense', containerName: 'OlySense' }
      },
      {
        containerId: 'OlySense',
        containerType: 'ado',
 ...
 * specs/OlySense/platform-engineering/
